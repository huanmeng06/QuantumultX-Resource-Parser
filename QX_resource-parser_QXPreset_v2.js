/** 
☑️ 资源解析器 ©𝐒𝐡𝐚𝐰𝐧  ⟦2026-05-22 16:38⟧
----------------------------------------------------------
...
// Enhanced comments by QX CN-TW fusion build
var $parser = $parser || {};
    ...
    sections: [
      {
        ...
        title: "✈️ 订阅获取",
        items: [
          { type: "switch", key: "UA", label: "✈️ UA 替换",
            onValue: "1", offValue: "",
            description: "\n尝试使用 Shadowrocket 的 User-Agent 重新获取订阅内容" }
        ]
      },
      {
        type: "group",
        ...
        items: [
          { type: "tags", key: "in", label: "✅ 保留（in）",
            description: "\n按节点名关键字保留。每行一个关键字表示“或”；同行用 . 分隔表示“与”。例：另起一行填 香港、中国台湾 表示含其一即可；同一行填 香港.IPLC 表示同时含香港和 IPLC。",
            placeholder: "如：香港 / 香港.IPLC" },
          { type: "tags", key: "out", label: "🗑 删除（out）",
            description: "\n按节点名关键字删除。每行一个关键字表示“或”；同行用 . 分隔表示“与”。",
            placeholder: "如：BGP / BGP.试用" },
          { type: "text", key: "regex", label: "🔎 正则保留（regex）",
            description: "\n对节点完整信息正则匹配以保留节点。匹配范围包括协议、端口、加密、host 等字段。",
            placeholder: "iplc" },
          { type: "text", key: "regout", label: "🚫 正则删除（regout）",
            description: "\n对节点完整信息正则匹配以删除节点。",
            placeholder: "" },
          { type: "text", key: "sort", label: "↕️ 节点排序（sort）",
            description: "\n对节点进行排序。参数为 1 正序、-1 逆序、x 随机排序，或使用 > / < 连接自定义规则。",
            placeholder: "🇭🇰>🇯🇵>🇨🇳>🇺🇸" }
        ]
      },
      {
        ...
        items: [
          { type: "select", key: "emoji", label: "🎌 Emoji 旗帜",
            description: "\n添加或删除节点名中的地区旗帜。中国台湾相关关键词统一使用 🇨🇳。",
            options: _emojiOptions() },
          { type: "text", key: "replace", label: "🔁 正则替换节点信息（replace）",
            description: "\n替换节点完整信息。可匹配所有内容，格式为 regex1@str1+regex2@str2。",
            placeholder: "" },
          ...
          { type: "switch", key: "cert", label: "🔐 TLS 证书验证",
            description: "\n开启或关闭 TLS 证书验证。默认关闭。",
            onValue: "1", offValue: "-1" },
          { type: "switch", key: "uot", label: "🧩 UDP over TCP",
            onValue: "1", offValue: "",
            description: "\n开启 udp-over-tcp=true。仅限 SS/SSR。" },
          { type: "switch", key: "aead", label: "🧬 VMess AEAD",
            description: "\n关闭 VMess AEAD 参数。",
            onValue: "", offValue: "-1" },
          { type: "text", key: "alpn", label: "🧷 ALPN",
            description: "\n指定 over-tls 节点的 ALPN 参数。",
            placeholder: "h2" },
          { type: "text", key: "host", label: "🌐 Host",
            description: "\n修改已有 host；如果需要新增 host，请用 ☠️ 结尾。",
            placeholder: "" },
          { type: "text", key: "checkurl", label: "🧪 Check URL",
            description: "\n指定 server_check_url 参数。",
      ...
      },
      {
        ...
        items: [
          ...
          { type: "switch", key: "cleantag", label: "🧼 清理节点名特殊符号",
            onValue: "1", offValue: "",
            description: "\n只保留常见中英文、数字、空格和少量安全符号。" },
          { type: "switch", key: "autoe", label: "🎌 自动地区 Emoji",
            onValue: "1", offValue: "",
            description: "\n根据节点名自动识别地区并添加 emoji。中国台湾相关关键词统一使用 🇨🇳。" },
          { type: "switch", key: "airport", label: "🏷 插入机场名前缀",
            onValue: "1", offValue: "",
            description: "\n使用 QX 资源标签作为 [机场名]，优先插入到地区 emoji 后面。" },
          { type: "switch", key: "rsort", label: "🧭 按地区顺序排序",
            onValue: "1", offValue: "",
            description: "\n默认顺序：香港、日本、新加坡、中国、美国，其余节点沉底。" }
        ]
      },
      {
        type: "group",
    ...
    sections: [
      {
        type: "group",
        title: "🔀 Filter「分流」",
        description: "\n🔀 Filter「分流」\n仅对 rewrite_remote / filter_remote 生效。",
        items: [
          { type: "tags", key: "in", label: "✅ 保留分流规则（in）",
            description: "\n按关键字保留 filter。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "tags", key: "out", label: "🗑 禁用分流/重写（out）",
            description: "\n按关键字禁用或删除 filter。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "text", key: "policy", label: "🎯 单策略组指定（policy）",
            description: "\nrule-set 生成策略组的名字，默认 Shawn。",
            placeholder: "Shawn" },
          { type: "text", key: "pset", label: "🧩 多策略组指定（pset）",
            description: "\n为不同关键词或正则分别指定策略组，格式为 regex1@policy1+regex2@policy2。" },
          { type: "text", key: "regex", label: "🔎 正则保留（regex）",
            description: "\n正则匹配并保留指定内容。" },
          { type: "text", key: "regout", label: "🚫 正则删除（regout）",
            description: "\n正则匹配并删除指定内容。" },
          { type: "text", key: "replace", label: "🔁 正则替换（replace）",
            description: "\n正则替换指定内容，格式为 regex@new_regex。" }
        ]
      },
      {
        type: "group",
        ...
        items: [
          { type: "text", key: "via", label: "🔌 via-interface（规则订阅）",
            description: "\n0 表示 via-interface=%TUN%。设置代理链时，规则分流需使用此参数，并在策略偏好将策略组指定为落地策略组/节点。",
            placeholder: "填写 0 即可" },
          { type: "text", key: "relay", label: "🔗 代理链 Relay（节点订阅）",
            description: "\n将落地节点订阅转换为 ip/host 规则，指向中转策略组/节点。设置 Relay 时，请关闭该订阅首页的策略偏好设置。via 与 relay 不应在同一条引用中同时设置。",
            placeholder: "填写你的中转策略组/节点名" }
        ]
      },
      {
        type: "group",
        ...
        items: [
          ...
          { type: "switch", key: "cdn", label: "🚀 GitHub CDN 加速",
            onValue: "1", offValue: "",
            description: "\n将 GitHub 脚本地址转换为 CDN 链接。" },
          { type: "select", key: "dst", label: "🔄 转换目标（dst）",
            description: "\n指定转换目标为 rewrite 或 filter。",
            options: [
              { label: "默认（不转换）", value: "" },
          ...
          { type: "select", key: "fcr", label: "📶 网络接口指定（fcr）",
            description: "\n为分流规则添加 force-cellular、multi-interface 或 multi-interface-balance 参数。",
            options: [
              { label: "默认", value: "" },
    ...
    sections: [
      {
        ...
        items: [
          { type: "tags", key: "in", label: "✅ 保留分流/重写（in）",
            description: "\n按关键字保留 rewrite/filter。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "tags", key: "out", label: "🗑 禁用分流/重写（out）",
            description: "\n按关键字禁用或删除 rewrite/filter。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "tags", key: "inhn", label: "🌐 保留主机名（inhn）",
            description: "\n按关键字保留相关 hostname。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "tags", key: "outhn", label: "🚫 删除主机名（outhn）",
            description: "\n按关键字删除相关 hostname。每行一个关键字，多行之间为“或”关系。",
...
function get_emoji(emojip, sname) {
   var Lmoji = { 
    ...
    "🇨🇳": ["CN", "China", "回国", "中国","中國", "江苏", "北京", "上海", "广州", "深圳", "杭州", "徐州", "青岛", "宁波", "镇江", "back", "TW", "Taiwan", "TAIWAN", "台湾", "台北", "台中", "新北", "彰化", "CHT", "台", "HINET"],
    "🇨🇺": ["古巴"],
    "🇸🇲": ["圣马力诺","聖馬利諾"],
    "🇰🇾": ["开曼群岛","開曼群島"],
    "🇫🇯": ["斐济","斐濟"],
