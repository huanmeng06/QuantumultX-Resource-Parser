...
----------------------------------------------------------
🧩 融合版说明（CN_TW_UI_EmojiBullets）
❶ 本版以原作者 2026-05-18 版为主干，保留 AnyTLS、UA 重试、rewrite/filter 转换等原功能。
❷ 新增 QX 自定义增强：qxf=1 可一键启用伪节点清理、节点名清理、地区 Emoji、机场名前缀、地区排序。
❸ 台湾相关节点在自定义 Emoji 与排序逻辑中统一使用 🇨🇳。
❹ 默认不启用自定义增强；不填 qxf/clean/autoe/airport/rsort 时，尽量保持原作者默认行为。
❺ 可视化说明区已改成 Emoji 分点，便于在 Quantumult X 参数界面快速阅读。
----------------------------------------------------------
...
// ==== UI formatting note ====
// 本版已优化 Quantumult X 918+ 可视化说明：
// 1) 每个选项的说明都会在“选项名：”后自动换行；
// 2) 选项名前增加 Emoji，便于在 QX 深色界面中快速扫读；
// 3) QX 可视化说明通常不支持真正加粗，因此用 Emoji、换行和短句增强层级。
// ============================

// ==== Parser Helper UI by Quantumult-X (918+) ==== 
// 说明：这是 Quantumult X 新版“解析器参数可视化”的声明区。
// 它只负责把 # 后面的参数做成可点击的开关、输入框、下拉框；不直接参与节点解析。
//
// 三个函数的职责：
// 1. hashSchema()：告诉 QX 界面要显示哪些参数、每个参数的标题、说明、默认选项。
// 2. hashToUI(hash)：把已有链接里的 #a=1&b=xxx 解析成界面里已经填好的值。
// 3. uiToHash(values)：把你在界面里勾选/填写的内容重新写回 #a=1&b=xxx。
//
// 改说明文字、分组标题、placeholder，只需要改 hashSchema()；不要随便改 hashToUI/uiToHash。
var $parser = $parser || {};

// schema：作者声明的全部可视化参数（不带 value）。
// 注意：这里的 key 必须和下方真正解析逻辑里的参数名一致，例如 qxf、in、out、emoji。
$parser.hashSchema = function () {
  // helper 必须定义在函数体内部 —— QX 协议要求三个函数自包含。
  function _emojiOptions() {
    return [
      { label: "添加地区 Emoji", value: "1"  },
      { label: "国行设备兼容", value: "2"  },
      { label: "删除已有 Emoji", value: "-1" }
    ];
  }
  function _ptnOptions() {
    return [
      ...
      { label: "𝐀 加粗大写", value: "3" },
      { label: "𝗮 加粗小写", value: "4" },
      { label: "𝔸 双线大写", value: "5" },
      { label: "𝕒 双线小写", value: "6" },
      { label: "ᵃ 小写上标", value: "7" },
  ...
  function _nptOptions() {
    return [
      ...
      { label: "❶ 实心圆数字", value: "2" },
      { label: "⓵ 双圈数字", value: "3" },
      { label: "𝟙 双线数字", value: "4" },
      { label: "¹ 上标数字", value: "5" },
      { label: "₁ 下标数字", value: "6" },
      { label: "𝟏 加粗数字", value: "7" },
      { label: "𝟷 等宽数字", value: "8" }
    ];
  }

  if ($resource.type == "server") { // 节点订阅：server_remote / 节点列表
    return {
    version: 1,
    sections: [
      {
        type: "group",
        title: "🚀 快速上手",
        description: "✅ 新手只需要看这一组。\n⚠️ 一般推荐：UA=1 解决获取异常；\n🧩 qxf=1 启用自动清洗；\n📊 info=1 显示流量。",
        items: [
          { type: "switch", key: "UA", label: "📡 UA 替换：用 Shadowrocket UA 重新获取订阅",
            onValue: "1", offValue: "",
            description: "\n📡 订阅返回空白、Unsupported Media Type、HTML 页面或乱码时打开。\n🔸 正常订阅可以不打开。" },
          { type: "switch", key: "qxf", label: "✅ 一键启用 QX 自定义增强",
            onValue: "1", offValue: "",
            description: "\n✅ 新手推荐打开。\n🔢 等价于 clean=1&cleantag=1&autoe=1&airport=1&rsort=1。\n🔢 只整理节点名和排序，不修改服务器地址、端口、密码。" },
          { type: "switch", key: "info", label: "📊 显示订阅流量/到期时间",
            onValue: "1", offValue: "",
            description: "\n📊 机场提供流量信息时，会用通知显示已用流量、剩余流量、到期时间。" }
        ]
      },
      {
        type: "group",
        title: "🎲 筛选与排序",
        description: "🗑️ 先用 in/out 做普通关键词筛选；\n🗑️ 只有需要匹配协议、端口、加密方式等完整内容时，再用 regex/regout。",
        items: [
          { type: "tags", key: "in", label: "🎯 保留节点（in）",
            description: "\n🎯 只保留节点名包含指定关键词的节点。\n🔸 多行之间是“或”；\n🔸 同一行用 . 表示“与”。\n💡 例：一行填 香港，另一行填 日本 = 保留香港或日本；\n🔸 填 香港.IPLC = 同时包含香港和 IPLC。",
            placeholder: "香港\n日本\n香港.IPLC" },
          { type: "tags", key: "out", label: "🗑️ 删除节点（out）",
            description: "\n🗑️ 删除节点名包含指定关键词的节点。\n📊 适合删试用、倍率、官网、剩余流量、过期提示等。\n🔸 多行之间是“或”；\n🔸 同一行用 . 表示“与”。",
            placeholder: "倍率\n剩余流量\n官网" },
          { type: "text", key: "regex", label: "🔎 正则保留（regex）",
            description: "\n🎯 正则匹配节点完整内容，匹配成功才保留。\n🔐 可匹配协议、端口、加密、host 等字段。\n🔍 不会写正则时优先用 in。",
            placeholder: "iplc|iepl" },
          { type: "text", key: "regout", label: "🚫 正则删除（regout）",
            description: "\n🗑️ 正则匹配节点完整内容，匹配成功就删除。\n🗑️ 适合批量删某些协议、端口、倍率或关键词。\n⚠️ 慎用，写错可能删空订阅。",
            placeholder: "expire|traffic|剩余流量" },
          { type: "text", key: "sort", label: "🔢 节点排序（sort）",
            description: "\n✏️ 1=按名称正序；\n🔸 -1=倒序；\n🔸 x=随机；\n🔢 也可写自定义顺序。\n🔢 > 表示这些关键词排前面，< 表示这些关键词排后面。",
            placeholder: "🇭🇰>🇯🇵>🇸🇬>🇨🇳>🇺🇸" }
        ]
      },
      {
        type: "group",
        title: "🧩 QX 自定义增强（可拆分使用）",
        description: "🧩 qxf=1 会一次打开本组全部功能。\n🔢 若只想要其中一部分，可以单独打开 clean、cleantag、autoe、airport、rsort。\n🏳️ 台湾相关关键词统一显示 🇨🇳。",
        items: [
          { type: "switch", key: "clean", label: "🧹 删除说明/流量/官网/群组等伪节点",
            onValue: "1", offValue: "",
            description: "\n💡 自动删除不像真实节点的条目，例如剩余流量、到期时间、官网、群组、更新提示、分割线等；\n🎯 会尽量保留防失联、备用、UDP、TCP 这类可能有用的节点。" },
          { type: "switch", key: "cleantag", label: "🧽 清理节点名特殊符号",
            onValue: "1", offValue: "",
            description: "\n⚠️ 删除节点名里的多余装饰符号、异常字符和杂乱空格，只保留中文、英文、数字、空格以及 - . _ () [] | 等常用符号。" },
          { type: "switch", key: "autoe", label: "🏳️ 自动识别地区 Emoji",
            onValue: "1", offValue: "",
            description: "\n🏳️ 根据节点名里的国家/地区关键词自动添加旗帜。\n🏳️ 中国台湾相关节点统一使用 🇨🇳。\n🏳️ 如果你还开了原版 emoji=1，建议先测试是否重复。" },
          { type: "switch", key: "airport", label: "🏷️ 插入机场名前缀",
            onValue: "1", offValue: "",
            description: "\n🏷️ 把 QX 资源名称作为 [机场名] 插入节点名。\n✏️ 若节点名前已有 Emoji，则放在 Emoji 后面。\n🏷️ 适合同时导入多个机场时区分来源。" },
          { type: "switch", key: "rsort", label: "🔢 按地区顺序排序",
            onValue: "1", offValue: "",
            description: "\n🔢 按常用地区顺序排列：香港、日本、新加坡、中国、美国，其他地区放后面。\n🏳️ 中国台湾相关节点归入中国。" }
        ]
      },
      {
        type: "group",
        title: "⛰️ 节点参数调整",
        description: "🔸 这些参数会改节点配置字段。\n⚠️ 不了解含义时不要全开；\n🔐 多数情况下只需要 UDP、TFO、证书验证、Check URL。",
        items: [
          { type: "select", key: "emoji", label: "🏳️ 原版 Emoji 旗帜处理",
            description: "\n🔸 原作者内置 Emoji 处理。\n🗑️ 添加/删除节点名中的地区旗帜。\n⚠️ 若已开启 autoe，建议谨慎同时开启，避免重复处理。",
            options: _emojiOptions() },
          { type: "text", key: "replace", label: "🔁 正则替换节点完整内容（replace）",
            description: "\n🔸 高级功能。\n🔍 格式：正则@替换内容，多组用 + 连接。\n✏️ 它会匹配节点完整内容，可改节点名，也可能改协议字段；\n⚠️ 写错会导致节点不可用。",
            placeholder: "旧字段@新字段" },
          { type: "switch", key: "udp", label: "🔗 UDP Relay",
            onValue: "1", offValue: "-1",
            description: "\n🔗 打开=强制 udp-relay=true；\n🔗 关闭=强制 udp-relay=false。\n🔗 游戏、语音、部分代理链可能需要 UDP。" },
          { type: "switch", key: "tfo", label: "⚡ Fast Open",
            onValue: "1", offValue: "-1",
            description: "\n🔸 打开=fast-open=true；\n🔸 关闭=fast-open=false。\n🔐 是否有效取决于系统、网络和节点服务端支持。" },
          { type: "switch", key: "cert", label: "🔐 TLS 证书验证",
            onValue: "1", offValue: "-1",
            description: "\n🔐 打开=校验证书；\n🔐 关闭=不校验证书。\n⚙️ 默认通常关闭。\n✅ 安全性优先可打开；\n⚠️ 部分自签或异常证书节点打开后可能不可用。" },
          { type: "switch", key: "uot", label: "🔀 UDP over TCP（仅 SS/SSR）",
            onValue: "1", offValue: "",
            description: "\n🔸 给 SS/SSR 节点添加 udp-over-tcp=true。\n🔸 只在你明确需要 UDP over TCP 时开启。" },
          { type: "switch", key: "aead", label: "🧬 VMess AEAD",
            onValue: "", offValue: "-1",
            description: "\n🔸 关闭 VMess AEAD。\n⚠️ 只有旧 VMess 节点兼容性异常时才考虑关闭。" },
          { type: "text", key: "alpn", label: "💡 ALPN",
            description: "\n💡 给 over-tls 类型节点指定 ALPN，例如 h2 或 http/1.1。\n⚠️ 不了解服务端配置时留空。",
            placeholder: "h2" },
          { type: "text", key: "host", label: "🔐 Host / SNI / 混淆 Host",
            description: "\n🔐 修改已有 host。\n🔐 若要新增 host，可在内容后加 ☠️。\n🏷️ 该项会影响连接，必须与机场配置一致。",
            placeholder: "example.com" },
          { type: "text", key: "checkurl", label: "🌐 测速 URL（server_check_url）",
            description: "\n🔐 指定节点可用性测试地址。\n🌐 建议使用稳定、轻量、可直连或可代理访问的 URL。",
            placeholder: "http://www.gstatic.com/generate_204", keyboard: "url" }
        ]
      },
      {
        type: "group",
        title: "🎯 重命名",
        description: "✏️ rename 先按你写的规则改名；\n✅ rrname 通常用于 Emoji 处理后再次改名。\n🧩 只想简单整理名称时，优先用 qxf 或 cleantag。",
        items: [
          { type: "text", key: "rename", label: "✏️ 节点重命名（rename）",
            description: "\n🏷️ 格式：旧名@新名、前缀@、@后缀。\n🔐 多组用 + 连接。\n🗑️ 字段后加 ☠️ 表示删除该字段。\n💡 例：香港@HK+@[1X]+倍率☠️。",
            placeholder: "香港@HK+@[1X]+倍率☠️" },
          { type: "text", key: "rrname", label: "🏳️ Emoji 后重命名（rrname）",
            description: "\n🏳️ 在原版 emoji 处理之后再执行一次重命名。\n🔸 适合想让 Emoji 保持在最前，同时继续改后面的文字。",
            placeholder: "香港@HK" },
          { type: "select", key: "ptn", label: "🔤 英文字母样式（ptn）",
            description: "\n✏️ 把节点名里的英文字母转换成花式字符。\n🔸 主要是美化用途；\n🔸 过度使用可能降低可读性。",
            options: _ptnOptions() },
      ...
      },
      {
        type: "group",
        title: "🧪 其他与调试",
        description: "⚠️ 解析异常、想看结果、或需要手动指定资源类型时再使用。",
        items: [
          { type: "select", key: "ntf", label: "🔔 解析通知（ntf）",
            options: [
              { label: "默认", value: "" },
              { label: "关闭通知", value: "0"},
              { label: "打开通知", value: "1"}
            ],
            description: "\n🔸 控制解析器通知。\n⚙️ 节点订阅默认偏安静；\n🗑️ rewrite/filter 默认会提示，防止误删规则。" },
          { type: "text", key: "flow", label: "📊 手动流量参数（flow）",
            description: "\n📊 机场没有返回流量信息时可手动写。\n💡 格式：到期日期:总流量GB:已用GB，例如 2026-12-31:1000:54。",
            placeholder: "2026-12-31:1000:54" },
          { type: "select", key: "type", label: "⚙️ 强制资源类型（type）",
            description: "\n⚙️ 自动识别失败时使用。\n🔸 节点订阅选 Nodes；\n🔸 分流规则选 Rule；\n🔸 模块选 Module；\n⚙️ domain-set/list 按资源实际类型选择。",
            options: [
              { label: "自动识别", value: "" },
              { label: "Nodes 节点订阅", value: "nodes" },
              { label: "Rule 分流规则", value: "rule" },
              { label: "Module 模块", value: "module" },
              { label: "List 规则列表", value: "list" },
              { label: "Domain Set 域名集合", value: "domain-set" }
            ] }
        ]
      }
    ]
  };
} else if ($resource.type == "filter") { // 分流规则：filter_remote / rule-set
  return {
    version: 1,
    sections: [
      {
        type: "group",
        title: "🔀 Filter 分流规则",
        description: "🔸 用于 filter_remote、rule-set、list 等分流规则资源。\n🧭 常用：policy 指定策略组；\n🗑️ in/out 过滤规则；\n🌐 cdn 转换 GitHub 链接。",
        items: [
          { type: "tags", key: "in", label: "🎯 保留分流规则（in）",
            description: "\n🎯 只保留包含指定关键词的分流规则。\n🔸 多行之间是“或”。\n💡 例如填 weibo，只保留含 weibo 的规则。",
            placeholder: "weibo\ntelegram" },
          { type: "tags", key: "out", label: "🗑️ 删除分流规则（out）",
            description: "\n🗑️ 删除包含指定关键词的分流规则。\n🔸 适合从远程规则里排除某些域名、APP 或脚本相关规则。",
            placeholder: "ads\ntracking" },
          { type: "text", key: "policy", label: "🧭 统一策略组（policy）",
            description: "\n🧭 把生成的分流规则统一指向这个策略组。\n⚙️ 默认是 Shawn。\n💡 常见写法：PROXY、DIRECT、Reject、你的节点选择组名。",
            placeholder: "PROXY" },
          { type: "text", key: "pset", label: "🧭 按关键词指定不同策略组（pset）",
            description: "\n🔍 格式：正则1@策略组1+正则2@策略组2。\n🔸 适合一份规则里不同关键词走不同策略。",
            placeholder: "Google@PROXY+Apple@DIRECT" },
          { type: "text", key: "regex", label: "🔎 正则保留（regex）",
            description: "\n🎯 正则匹配规则完整内容，匹配成功才保留。\n🔍 不会正则时优先用 in。",
            placeholder: "Google|Telegram" },
          { type: "text", key: "regout", label: "🚫 正则删除（regout）",
            description: "\n🗑️ 正则匹配规则完整内容，匹配成功就删除。\n⚠️ 慎用，写错可能删空规则。",
            placeholder: "ads|tracking" },
          { type: "text", key: "replace", label: "🔁 正则替换规则内容（replace）",
            description: "\n🔍 格式：正则@替换内容。\n🧭 可批量修改策略名、域名、规则类型等；\n⚠️ 高级功能，建议先备份。",
            placeholder: "旧策略@新策略" }
        ]
      },
      {
        type : "group",
        title: "🔗 代理链 Relay",
        description: "🔗 代理链场景使用。\n🔗 一般规则订阅用 via，节点订阅转规则用 relay；\n⚠️ 两者不要在同一条引用里同时设置。",
        items : [
          { type: "text", key: "via", label: "🔗 via-interface（规则订阅）",
            description: "\n🔗 给分流规则添加 via-interface。\n🔗 填写 0 表示 via-interface=%TUN%。\n🔗 设置代理链时，规则订阅通常需要这个参数。",
            placeholder: "0" },
          { type: "text", key: "relay", label: "🔗 Relay 目标策略（节点订阅转规则）",
            description: "\n🔐 把落地节点订阅转换成 ip/host 规则，并指向这里填写的中转策略组/节点名。\n🔸 使用时请关闭该订阅首页里的策略偏好设置。",
            placeholder: "中转策略组或节点名" }
        ]
      },
      {
        type: "group",
        ...
        items: [
          { type: "select", key: "ntf", label: "🔔 解析通知（ntf）",
            ...
            description: "\n🗑️ 规则类资源默认会提示解析结果，防止误删。" },
          { type: "select", key: "type", label: "⚙️ 强制资源类型（type）",
            description: "\n⚙️ 自动识别失败时使用。\n⚙️ rule/list/domain-set/module 按资源真实类型选择。",
            options: [
              { label: "自动识别", value: "" },
              { label: "Nodes 节点订阅", value: "nodes" },
              { label: "Rule 分流规则", value: "rule" },
          ...
          { type: "switch", key: "cdn", label: "🌐 GitHub CDN 加速",
            onValue: "1", offValue: "",
            description: "\n🌐 把 GitHub raw 脚本地址转换为 jsDelivr CDN 地址。\n🔸 部分网络下加载更稳定。" },
          { type: "select", key: "dst", label: "↔️ 转换目标（dst）",
            description: "\n🔸 强制把 module/rule-set 转成 rewrite 或 filter。\n⚙️ 一般保持默认；\n🔸 只有资源识别不符合预期时再改。",
            options: [
              { label: "默认：自动判断", value: "" },
              { label: "转为 Rewrite", value: "rewrite" },
              { label: "转为 Filter", value: "filter" }
            ] },
          { type: "select", key: "fcr", label: "📶 网络接口指定（fcr）",
            description: "\n🔸 给规则添加 force-cellular、multi-interface 或 multi-interface-balance。\n⚙️ 一般用户保持默认。",
            options: [
              { label: "默认", value: "" },
...
} else { // 重写规则：rewrite_remote / module 中的 rewrite
  return {
    version: 1,
    sections: [
      {
        type: "group",
        title: "🧱 Rewrite 重写规则",
        description: "🔐 用于 rewrite_remote 和模块中的 rewrite/hostname。\n🗑️ 常用：out 删除某些脚本，outhn 删除对应 hostname，cdn 加速 GitHub 脚本。",
        items: [
          { type: "tags", key: "in", label: "🎯 保留重写规则（in）",
            description: "\n🎯 只保留包含指定关键词的 rewrite/filter 规则。\n🔸 多行之间是“或”。",
            placeholder: "weibo\nyoutube" },
          { type: "tags", key: "out", label: "🗑️ 删除重写规则（out）",
            description: "\n🗑️ 删除包含指定关键词的 rewrite/filter 规则。\n💡 常用于禁用某个脚本，例如 tb_price.js、wb_ad.js。",
            placeholder: "tb_price.js\nwb_ad.js" },
          { type: "tags", key: "inhn", label: "🎯 保留 Hostname（inhn）",
            description: "\n🎯 只保留匹配关键词的 hostname。\n🔸 通常和 in 配合使用。",
            placeholder: "weibo" },
          { type: "tags", key: "outhn", label: "🗑️ 删除 Hostname（outhn）",
            description: "\n🗑️ 删除匹配关键词的 hostname。\n🗑️ 禁用某些重写脚本时，建议同时删除对应 hostname。",
            placeholder: "taobao\nweibo" },
          { type: "text", key: "regex", label: "🔎 正则保留（regex）",
            description: "\n🎯 正则匹配 rewrite/filter/hostname 完整内容，匹配成功才保留。\n⚠️ 慎用。",
            placeholder: "weibo|taobao" },
          { type: "text", key: "regout", label: "🚫 正则删除（regout）",
            description: "\n🗑️ 正则匹配 rewrite/filter/hostname 完整内容，匹配成功就删除。\n⚠️ 慎用，写错可能删空。",
            placeholder: "ads|tracking" },
          { type: "text", key: "replace", label: "🔁 正则替换（replace）",
            description: "\n🔍 格式：正则@替换内容。\n🔍 可把脚本 URL、参数或策略字段替换成你指定的内容。",
            placeholder: "旧脚本@新脚本" }
        ]
      },
      {
        ...
        items: [
          { type: "select", key: "ntf", label: "🔔 解析通知（ntf）",
            ...
            description: "\n🗑️ 重写类资源默认会提示解析结果，防止误删。" },
          { type: "select", key: "type", label: "⚙️ 强制资源类型（type）",
            description: "\n⚙️ 自动识别失败时使用。\n⚙️ rewrite/module/rule/list/domain-set 按资源真实类型选择。",
            options: [
              { label: "自动识别", value: "" },
              { label: "Nodes 节点订阅", value: "nodes" },
              { label: "Rule 分流规则", value: "rule" },
...
function ResourceParse() {
  ...
  if (flag == 1) { //server 类型统一处理
    ...
    // QX 自定义增强插入点：
    // - 放在 in/out、regex/regout、sfilter、rrname 之后，先让原作者逻辑完成基础筛选。
    // - 放在 rename、replace、emoji、tag_handle、TagCheck_QX 之前，避免破坏最终格式规范化。
    // - 默认不执行；只有 qxf=1 或单独开启 clean/cleantag/autoe/airport/rsort 时才执行。
    if (PQXFull == "1" || PQXClean == "1" || PQXCleanTag == "1" || PQXAutoEmoji == "1" || PQXAirport == "1" || PQXRegionSort == "1") {
      total = customFilterAndRename(total, {
        all: PQXFull,           // qxf=1：一键启用全部自定义增强
        filter: PQXClean,       // clean=1：删除流量/官网/群组/更新等伪节点
        cleanTag: PQXCleanTag,  // cleantag=1：清理节点名特殊符号
        emoji: PQXAutoEmoji,    // autoe=1：按地区关键词自动补 Emoji
        airport: PQXAirport,    // airport=1：插入 [机场名] 前缀
        sort: PQXRegionSort     // rsort=1：按常用地区顺序排序
      });
    }
...
// ==== QX 自定义节点清洗/命名增强（融合版）====
// 设计目标：只做“节点列表整理”，不改服务器地址、端口、密码、协议核心字段。
// 推荐用法：#qxf=1 一键启用；或拆分使用 clean=1&cleantag=1&autoe=1&airport=1&rsort=1。
// 执行顺序：过滤伪节点 -> 清理节点名 -> 自动 Emoji -> 插入机场前缀 -> 地区排序。
// 注意：台湾相关关键词在本自定义增强中统一输出 🇨🇳，并在排序中归入中国。
function customFilterAndRename(servers, options) {
    options = options || {};
    const enabledAll = _customFlag(options.all);
    const enabledFilter = enabledAll || _customFlag(options.filter);
    const enabledCleanTag = enabledAll || _customFlag(options.cleanTag);
    const enabledEmoji = enabledAll || _customFlag(options.emoji);
    const enabledAirport = enabledAll || _customFlag(options.airport);
    const enabledSort = enabledAll || _customFlag(options.sort);

    if (!Array.isArray(servers) || servers.length === 0) return servers;

    const excludeRegex = /(Data Left|Remain:|Traffic:|Expir[ey]|Reset|(\d[\d.]*\s*[MG]B[^\dA-Za-z]+|[:：]\s*)\d[\d.]*\s*GB(?![\dA-Za-z])|剩[余餘]流量|流量：|[到过過效]期|[时時][间間]|重置|分割线|残り使用容量|残りデータ通信量|有効期限|リセット|🔰 (ID|HSD|SNI):|📝 Gói:|最新[网網][站址]|官[方网]|获取|地址|群|更新)/i;
    const keepRegex = /(防失联|防失聯|备用|備用|UDP|TCP)/i;

    const emojiRules = [
        { emoji: "🇭🇰", regex: /(香港|Hong Kong|\bHK\b|深港|沪港|京港)(?!中[轉转])/i },
        { emoji: "🇨🇳", regex: /([台臺][湾灣北]|新[北竹]|彰化|高雄|Taiwan|\bTW\b)(?!中[轉转])/i },
        { emoji: "🇨🇳", regex: /(中国|中國|大陆|大陸|北京|上海|深圳|广州|廣州|杭州|南京|China|\bCN\b)(?!中[轉转])/i },
        { emoji: "🇯🇵", regex: /(日本|东京|大阪|名古屋|埼玉|福冈|Japan|\bJP\b|川日|泉日|沪日|深日)(?!中[轉转])/i },
        { emoji: "🇸🇬", regex: /(新加坡|[狮獅]城|Singapore|\bSG\b)(?!中[轉转])/i },
        { emoji: "🇰🇷", regex: /(朝[鲜鮮]|[韩韓][国國]|首尔|春川|Korea|\bKR\b)(?!中[轉转])/i },
        { emoji: "🇲🇾", regex: /(马来西亚|大马|吉隆坡|Malaysia|\bMY\b)(?!中[轉转])/i },
        { emoji: "🇮🇩", regex: /(印尼|印度尼西亚|雅加达|Indonesia|\bID\b)(?!中[轉转])/i },
        { emoji: "🇮🇳", regex: /(印度(?!尼西亚)|孟买|新德里|India|\bIN\b)(?!中[轉转])/i },
        { emoji: "🇵🇭", regex: /(菲律宾|马尼拉|Philippines|\bPH\b)(?!中[轉转])/i },
        { emoji: "🇹🇭", regex: /(泰国|曼谷|Thailand|\bTH\b)(?!中[轉转])/i },
        { emoji: "🇻🇳", regex: /(越南|胡志明|河内|Vietnam|\bVN\b)(?!中[轉转])/i },
        { emoji: "🇰🇿", regex: /(哈萨克斯坦|阿拉木图|阿斯塔纳|Kazakhstan|\bKZ\b)(?!中[轉转])/i },
        { emoji: "🇵🇰", regex: /(巴基斯坦|伊斯兰堡|Pakistan|\bPK\b)(?!中[轉转])/i },
        { emoji: "🇬🇧", regex: /(英[国國]|英格兰|伦敦|加的夫|曼彻斯特|伯克郡|United Kingdom|Great Britain|\bUK\b|\bGB\b)(?!中[轉转])/i },
        { emoji: "🇫🇷", regex: /(法[国國]|巴黎|马赛|斯特拉斯堡|France|\bFR\b)(?!中[轉转])/i },
        { emoji: "🇩🇪", regex: /(德[国國]|法兰克福|柏林|杜塞尔多夫|Germany|\bDE\b)(?!中[轉转])/i },
        { emoji: "🇳🇱", regex: /(荷兰|尼德兰|阿姆斯特丹|Netherlands|\bNL\b)(?!中[轉转])/i },
        { emoji: "🇷🇺", regex: /(俄[国國]|俄[罗羅]斯|莫斯科|圣彼得堡|西伯利亚|伯力|哈巴罗夫斯克|Russia|\bRUA?\b)(?!中[轉转])/i },
        { emoji: "🇨🇭", regex: /(瑞士|苏黎世|日内瓦|Switzerland|\bCH\b)(?!中[轉转])/i },
        { emoji: "🇸🇪", regex: /(瑞典|斯德哥尔摩|Sweden|\bSE\b)(?!中[轉转])/i },
        { emoji: "🇮🇹", regex: /(意大[利里]|米兰|罗马|Italy|\bIT\b)(?!中[轉转])/i },
        { emoji: "🇪🇸", regex: /(西班牙|马德里|Spain|\bES\b)(?!中[轉转])/i },
        { emoji: "🇵🇱", regex: /(波兰|华沙|Poland|\bPL\b)(?!中[轉转])/i },
        { emoji: "🇺🇦", regex: /(乌克兰|基辅|Ukraine|\bUA\b)(?!中[轉转])/i },
        { emoji: "🇦🇹", regex: /(奥地利|维也纳|Austria|\bAT\b)(?!中[轉转])/i },
        { emoji: "🇮🇪", regex: /(爱尔兰|都柏林|Ireland|\bIE\b)(?!中[轉转])/i },
        { emoji: "🇲🇩", regex: /(摩尔多瓦|基希讷乌|蒂拉斯波尔|德涅斯特|Moldova|Chisinau|Kishinev|Tiraspol|\bMD\b)(?!中[轉转])/i },
        { emoji: "🇺🇸", regex: /(美[国國]|华盛顿|波特兰|达拉斯|俄勒冈|凤凰城|菲尼克斯|费利蒙|弗里蒙特|硅谷|旧金山|拉斯维加斯|洛杉|圣何塞|圣荷西|圣塔?克拉拉|西雅图|芝加哥|哥伦布|纽约|阿什本|纽瓦克|丹佛|加利福尼亚|弗吉尼亚|马纳萨斯|俄亥俄|得克萨斯|[佐乔]治亚|亚特兰大|佛罗里达|迈阿密|United States|\bUSA?\b)(?!中[轉转])/i },
        { emoji: "🇨🇦", regex: /(加拿大|[枫楓][叶葉]|多伦多|蒙特利尔|温哥华|卡尔加里|Canada|\bCA\b)(?!中[轉转])/i },
        { emoji: "🇦🇷", regex: /(阿根廷|布宜诺斯艾利斯|Argentina|\bAR\b)(?!中[轉转])/i },
        { emoji: "🇧🇷", regex: /(巴西|圣保罗|里约|Brazil|\bBR\b)(?!中[轉转])/i },
        { emoji: "🇲🇽", regex: /(墨西哥|Mexico|\bMX\b)(?!中[轉转])/i },
        { emoji: "🇨🇱", regex: /(智利|圣地亚哥|Chile|\bCL\b)(?!中[轉转])/i },
        { emoji: "🇹🇷", regex: /(土耳其|伊斯坦布尔|Turkey|\bTR\b)(?!中[轉转])/i },
        { emoji: "🇦🇪", regex: /(阿联酋|迪拜|阿拉伯联合酋长国|United Arab Emirates|Dubai|\bAE\b)(?!中[轉转])/i },
        { emoji: "🇮🇱", regex: /(以色列|特拉维夫|耶路撒冷|Israel|\bIL\b)(?!中[轉转])/i },
        { emoji: "🇸🇦", regex: /(沙特|利雅得|Saudi Arabia|\bSA\b)(?!中[轉转])/i },
        { emoji: "🇿🇦", regex: /(南非|约翰内斯堡|South Africa|\bZA\b)(?!中[轉转])/i },
        { emoji: "🇪🇬", regex: /(埃及|开罗|Egypt|\bEG\b)(?!中[轉转])/i },
        { emoji: "🇳🇬", regex: /(尼日利亚|拉各斯|阿布贾|Nigeria|\bNG\b)(?!中[轉转])/i },
        { emoji: "🇦🇺", regex: /(澳大利亚|澳洲|悉尼|墨尔本|Australia|\bAU\b)(?!中[轉转])/i },
        { emoji: "🇳🇿", regex: /(新西兰|奥克兰|New Zealand|\bNZ\b)(?!中[轉转])/i },
        { emoji: "🌍", regex: /(Anycast|BGP|Global)/i }
    ];

    const regionOrder = ["🇭🇰", "🇯🇵", "🇸🇬", "🇨🇳", "🇺🇸"];
    const leadingEmojiRegex = /^((?:[\u{1F1E6}-\u{1F1FF}]{2}|[\u{1F300}-\u{1FAFF}]|[\u{2600}-\u{27BF}])\s*)+/u;

    function sanitizeName(name) {
        const leading = name.match(leadingEmojiRegex);
        const prefixEmoji = leading ? leading[0].trim() : "";
        const body = name.replace(leadingEmojiRegex, "").replace(/[^\u4e00-\u9fa5a-zA-Z0-9\s\-\.\_\(\)\[\]\|]/g, "").replace(/\s+/g, " ").trim();
        return prefixEmoji ? (prefixEmoji + (body ? " " + body : "")) : body;
    }

    function autoEmoji(name) {
        const body = name.replace(leadingEmojiRegex, "").trim();
        for (const rule of emojiRules) {
            if (rule.regex.test(body)) return `${rule.emoji} ${body}`.trim();
        }
        return name.trim();
    }

    function airportPrefix(name) {
        let airportName = "";
        if (typeof $resource !== "undefined" && $resource.tag) airportName = $resource.tag;
        else if (typeof subtag !== "undefined" && subtag) airportName = subtag;
        airportName = String(airportName || "").trim();
        if (!airportName || name.indexOf(`[${airportName}]`) !== -1) return name;

        const leading = name.match(leadingEmojiRegex);
        if (leading) {
            const emoji = leading[0].trim();
            const rest = name.slice(leading[0].length).trim();
            return `${emoji} [${airportName}] ${rest}`.trim();
        }
        return `[${airportName}] ${name}`.trim();
    }

    function getRegionWeight(tag) {
        for (let i = 0; i < regionOrder.length; i++) {
            if (tag.indexOf(regionOrder[i]) !== -1) return i;
        }
        for (let j = 0; j < emojiRules.length; j++) {
            if (emojiRules[j].regex.test(tag)) {
                const pos = regionOrder.indexOf(emojiRules[j].emoji);
                return pos !== -1 ? pos : 999;
            }
        }
        return 999;
    }
    ...
    for (let i = 0; i < servers.length; i++) {
        ...
        const match = server.match(/^(.*tag\s*=\s*)(.*)$/);
        if (!match) {
        ...
        if (enabledFilter && excludeRegex.test(tag) && !keepRegex.test(tag)) continue;

        let newTag = tag;
        if (enabledCleanTag) newTag = sanitizeName(newTag);
        if (enabledEmoji) newTag = autoEmoji(newTag);
        if (enabledAirport) newTag = airportPrefix(newTag);

        newServers.push(prefix + newTag);
    }

    if (enabledSort) {
        newServers.sort((a, b) => {
            const tagA = (a.match(/tag\s*=\s*(.+)$/) || [null, a])[1].trim();
            const tagB = (b.match(/tag\s*=\s*(.+)$/) || [null, b])[1].trim();
            const weightA = getRegionWeight(tagA);
            const weightB = getRegionWeight(tagB);
